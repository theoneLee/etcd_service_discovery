...
func main() {
	...
	flag.StringVar(&node, "node", "001", "")
	flag.StringVar(&host, "host", "localhost:1111", "")

	flag.Parse()

	//go run service_register.go -node="1234" -host="localhost:11111"
	ser, _ := NewServiceReg([]string{"127.0.0.1:2379"}, 5)
	ser.PutService("/node/"+node, host)
